...
export type DeviceStatusType = 'Advertised' | 'Approved' | 'Disabled'
...
export class Devices extends ThBaseHandler {
  public static baseEndpoint = '/api/v1/devices'
  endpoint: string
  http: Client
  ...
  async getAll (queryOrOptions?: DevicesQuery | undefined): Promise<DevicesResponse> {
    ...
    try {
      ...
      if (response.data.cursors?.after) {
        next = (): Promise<DevicesResponse> => this.getAll({ uri: response.data.cursors.after })
      }
  ...
  async meta (queryOrOptions?: DevicesQuery): Promise<DevicesResponse> {
    ...
    try {
      ...
      throw new DevicesMetaFailed(error.message, { error })
    }
  }
  ...
  async create (device: DeviceCreateBody): Promise<DeviceResponse> {
...
export class DevicesMetaFailed extends BaseError {
  public name = 'DevicesMetaFailed'
  constructor (
    public message: string = 'Could not fetch the devices meta',
    properties?: Record<string, unknown>
  ) {
    super(message, properties)
    Object.setPrototypeOf(this, DevicesMetaFailed.prototype)
  }
}
