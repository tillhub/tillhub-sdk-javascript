...
export class Contents extends ThBaseHandler {
  public static baseEndpoint = '/api/v1/contents'
  endpoint: string
  http: Client
  ...
  async meta (queryOrOptions?: ContentsQuery): Promise<ContentsResponse> {
    ...
    try {
      ...
      throw new ContentsMetaFailed(error.message, { error })
    }
  }
...
export class ContentsMetaFailed extends BaseError {
  public name = 'ContentsMetaFailed'
  constructor (
    public message: string = 'Could not fetch the content templates meta',
    properties?: Record<string, unknown>
  ) {
    super(message, properties)
    Object.setPrototypeOf(this, ContentsMetaFailed.prototype)
  }
}
