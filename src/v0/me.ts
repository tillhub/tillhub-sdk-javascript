...
export interface Me {
  ...
  role: string
  scopes: string[]
  // MMS: { [unzerId]: roleId[] } — the dashboards this user can access. Scopes
  // above are resolved for the account named by the `x-client-account` header.
...
export class Me extends ThBaseHandler {
  ...
  async get (clientAccount?: string | null): Promise<MeResponse> {
    ...
    try {
      ...
      throw new MeFetchFailed(error?.response?.data?.msg || error.message, { error })
    }
  }
}
