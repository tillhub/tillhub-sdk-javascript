...
describe('v3: AnalyticsReportsRevenues', () => {
  it('can get revenue analytics reports export', async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      mock
        .onGet(`https://api.tillhub.com/api/v3/analytics/${legacyId}/reports/revenues/grouped?${qs.stringify(query)}`)
        .reply(() => {
          return [
            200,
            {
  ...
  it('rejects on status codes that are not 200', async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      mock
        .onGet(
          `https://api.tillhub.com/api/v3/analytics/${legacyId}/reports/revenues/grouped?${qs.stringify(query)}`
        )
        .reply(() => {
          return [205]
        })
