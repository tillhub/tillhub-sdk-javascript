...
describe('v1: AppointmentReminders: can get appointment reminder', () => {
  it("Tillhub's appointmentReminders are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock
        .onGet(`https://api.tillhub.com/api/v1/notifications/appointment-reminders/${legacyId}`)
        .reply(() => {
          return [
            200,
            {
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.appointmentReminders().get()
    } catch (err: any) {
      expect(err.name).toBe('AppointmentReminderFetchFailed')
    }
  })
})
