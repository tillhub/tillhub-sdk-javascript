...
const appointmentReminderId = '1234'
...
describe('v1: AppointmentReminders: can patch appointment reminder', () => {
  it("Tillhub's appointmentReminder are instantiable", async () => {
    ...
    const { data } = await instance.patch(appointmentReminderId, appointmentReminder)
  ...
  it('rejects on status codes that are not 200', async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onPatch(`https://api.tillhub.com/api/v1/notifications/appointment-reminders/${legacyId}`).reply(() => {
        return [205]
      })
    }
    ...
    try {
      ...
      expect(err.name).toBe('AppointmentReminderPatchFailed')
    }
  })
})
