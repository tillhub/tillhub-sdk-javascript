...
describe('v2: AnalyticsReportsTransactionsItems: can get all the transactions items', () => {
  it("Tillhub's analyticsReportsTransactionsItems are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onGet(`https://api.tillhub.com/api/v2/analytics/${legacyId}/reports/transactions/items`).reply(function (config) {
        return [
          200,
          {
            count: 1,
    ...
    const analyticsReportsTransactionsItems = th.analyticsHandlers().analytics.reports.AnalyticsReportsTransactionsItems

    expect(analyticsReportsTransactionsItems).toBeInstanceOf(v2.analytics.reports.AnalyticsReportsTransactionsItems)
