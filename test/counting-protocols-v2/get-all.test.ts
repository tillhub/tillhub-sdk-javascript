...
describe('v2: AnalyticsReportsCountingProtocols: can get all the counting protocols', () => {
  it("Tillhub's analyticsReportsCountingProtocols are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onGet(`https://api.tillhub.com/api/v2/analytics/${legacyId}/reports/cashier_counting_protocols/overview`).reply(function (config) {
        return [
          200,
          {
            count: 1,
    ...
    const analyticsReportsCountingProtocols = th.analyticsHandlers().analytics.reports.AnalyticsReportsCountingProtocols

    expect(analyticsReportsCountingProtocols).toBeInstanceOf(v2.analytics.reports.AnalyticsReportsCountingProtocols)
