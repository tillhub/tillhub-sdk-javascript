...
describe('v0: Product addons: can get count number of all', () => {
  ...
  it("Tillhub's product addons are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onGet(`https://api.tillhub.com/api/v0/product_addons/${legacyId}/meta`).reply(() => {
        return [
          200,
          {
            count: 50,
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.productAddons().meta()
    } catch (err: any) {
      expect(err.name).toBe('ProductAddonGetMetaFailed')
    }
  })
})
