...
describe('v0: Submissions: can create submission', () => {
  it("Tillhub's webhooks are instantiable", async () => {
    ...
    const { data } = await submissions.create(branch.id, submission.id)
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      const th = await initThInstance()
      await th.submissions().create(branch.id, submission.id)
    } catch (err: any) {
      expect(err.name).toBe('SubmissionCreateFailed')
    }
  })
})
