...
describe('v0: Transactions: can alter the image', () => {
  it("Tillhub's transactions are instantiable", async () => {
    ...
    const { data } = await transactions.putImage(mockTransactionId, mockImage)
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.transactions().putImage(mockTransactionId, mockImage)
    } catch (err) {
      expect(err.name).toBe('TransactionsImagePutFailed')
    }
  })
})
