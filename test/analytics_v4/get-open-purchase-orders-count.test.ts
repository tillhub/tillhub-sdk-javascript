...
describe('v4: Analytics: can get open purchase orders count', () => {
  it("Tillhub's analytics are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock
        .onGet(`https://api.tillhub.com/api/v4/analytics/${legacyId}/open-purchase-orders/count`)
        .reply(() => {
          return [200, { results }]
        })
    }
    ...
    const { data } = await analytics.getOpenPurchaseOrdersCount()
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      const th = await initThInstance()
      await th.analyticsV4().getOpenPurchaseOrdersCount()
    } catch (err: any) {
      expect(err.name).toBe('AnalyticsGetOpenPurchaseOrdersCountFailed')
    }
  })
})
