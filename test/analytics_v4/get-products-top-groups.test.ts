...
describe('v4: Analytics: can get products top groups', () => {
  it("Tillhub's analytics are instantiable", async () => {
    ...
    const { data } = await analytics.getProductsTopGroups()
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      const th = await initThInstance()
      await th.analyticsV4().getProductsTopGroups()
    } catch (err: any) {
      expect(err.name).toBe('AnalyticsGetProductsTopGroupsFailed')
    }
  })
})
