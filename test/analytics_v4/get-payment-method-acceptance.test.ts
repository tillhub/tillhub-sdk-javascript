...
describe('v4: Analytics: can get payment method acceptance', () => {
  it("Tillhub's analytics are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onGet(`https://api.tillhub.com/api/v4/analytics/${legacyId}/oms/payment-methods/acceptance`).reply(() => {
        return [200, { results }]
      })
    }
    ...
    const { data } = await analytics.getPaymentMethodAcceptance()
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      const th = await initThInstance()
      await th.analyticsUodV4().getPaymentMethodAcceptance()
    } catch (err: any) {
      expect(err.name).toBe('AnalyticsGetPaymentMethodAcceptanceFailed')
    }
  })
})
