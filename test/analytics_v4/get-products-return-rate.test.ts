...
describe('v4: Analytics: can get products return rate', () => {
  it("Tillhub's analytics are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock
        .onGet(`https://api.tillhub.com/api/v4/analytics/${legacyId}/products-return-rate`)
        .reply(() => {
          return [200, { results }]
        })
    }
    ...
    const { data } = await analytics.getProductsReturnRate()
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      const th = await initThInstance()
      await th.analyticsV4().getProductsReturnRate()
    } catch (err: any) {
      expect(err.name).toBe('AnalyticsGetProductsReturnRateFailed')
    }
  })
})
