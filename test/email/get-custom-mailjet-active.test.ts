...
describe('v0: Email: can get custom mailjet active', () => {
  it("Tillhub's email handler can get custom mailjet active status", async () => {
    ...
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onGet(`https://api.tillhub.com/api/v0/email/${legacyId}/custom-mailjet-active`).reply(() => {
        return [
          200,
          {
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.email().getCustomMailjetActive()
    } catch (err: any) {
      expect(err.name).toBe('EmailCustomMailjetActiveGetFailed')
    }
  })
})
