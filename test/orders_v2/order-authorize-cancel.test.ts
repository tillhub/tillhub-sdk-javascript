...
describe('v2: Order Actions', () => {
  it("Tillhub's order actions are instantiable", async () => {
    ...
    const { data } = await orderActions.orderAuthorizeCancel(orderId, payload)
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      const th = await initThInstance()
      await th.orderActionsV2().orderAuthorizeCancel(orderId, payload)
    } catch (err: any) {
      expect(err.name).toBe('OrderAuthorizeCancelFailed')
    }
  })
})
