...
describe('v0: Storefronts: can get the status of the sync process', () => {
  it("Tillhub's products are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock
        .onGet(
          `https://api.tillhub.com/api/v0/storefronts/${legacyId}/${storeId}/sync/status`
        )
        .reply(() => {
          return [
            200,
    ...
    const { data } = await storefronts.syncStatus(storeId)
