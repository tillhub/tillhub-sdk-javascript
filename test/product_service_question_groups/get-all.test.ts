...
describe('v0: Product Service Question Group: can get all', () => {
  it("Tillhub's ProductServiceQuestionGroups are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onGet(`https://api.tillhub.com/api/v0/product_service_question_groups/${legacyId}`).reply(function (config) {
        return [
          200,
          {
            count: 1,
    ...
    const { data } = await productServiceQuestionGroups.getAll()
