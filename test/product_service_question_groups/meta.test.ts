...
describe('v0: Product Service Question Groups: can get count number of all product Service Question Groups', () => {
  ...
  it("Tillhub's productServiceQuestionGroups are instantiable", async () => {
    ...
    const { data } = await productServiceQuestionGroups.meta()
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.productServiceQuestionGroups().meta()
    } catch (err) {
      expect(err.name).toBe('ProductServiceQuestionGroupsGetMetaFailed')
    }
  })
})
