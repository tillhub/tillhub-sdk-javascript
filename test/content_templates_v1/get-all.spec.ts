...
describe('v1: Contents Templates: can get all the contents templates', () => {
  it("Tillhub's contents templates are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onGet(`https://api.tillhub.com/api/v1/content_templates/${legacyId}`).reply(() => {
        return [
          200,
          {
            count: 1,
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      const th = await initThInstance()
      await th.contentTemplatesV1().getAll()
    } catch (err: any) {
