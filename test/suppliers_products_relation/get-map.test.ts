...
describe('v0:SuppliersProductsRelation: can get map', () => {
  const legacyId = '4564'
  ...
  it("Tillhub's instantiable and can get", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onPost(`https://api.tillhub.com/api/v0/business-partner-products/${legacyId}/map`).reply(() => {
        return [
          200,
          {
            results: [{
    ...
    const { data, msg } = await SuppliersProductsRelation.getMap({
      productId: [productId]
    })
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.suppliersProductsRelation().getMap({
        productId: [productId]
      })
    } catch (err: any) {
      expect(err.name).toBe('SuppliersProductsRelationMapFailed')
    }
  })
})
