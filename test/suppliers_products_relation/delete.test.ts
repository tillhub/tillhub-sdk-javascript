...
describe('v0: SuppliersProductsRelation: can delete a supplier to product relation', () => {
  it('can delete one', async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock
        .onDelete(`https://api.tillhub.com/api/v0/business-partner-products/${legacyId}/${supplierId}/${productId}`)
        .reply(() => {
          return [
            200,
            {
    ...
    const { data, msg } = await suppliersProductsRelation.delete(supplierId, productId)
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.suppliersProductsRelation().delete(supplierId, productId)
    } catch (err: any) {
      expect(err.name).toBe('SuppliersProductsRelationDeleteFailed')
    }
  })
})
