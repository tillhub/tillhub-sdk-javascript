...
describe('v0:SuppliersProductsRelation: can get suppliers', () => {
  const legacyId = '4564'
  ...
  it("Tillhub's instantiable and can get", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onGet(`https://api.tillhub.com/api/v0/business-partner-products/${legacyId}/lookup/${productId}`).reply(() => {
        return [
          200,
          {
            results: [{
    ...
    const { data, msg } = await SuppliersProductsRelation.getSuppliers(productId)
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.suppliersProductsRelation().getSuppliers(productId)
    } catch (err: any) {
      expect(err.name).toBe('SuppliersProductsRelationSuppliersFailed')
    }
  })
})
