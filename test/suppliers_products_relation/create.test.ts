...
describe('v0: SuppliersProductsRelation: can create a supplier to product relation', () => {
  ...
  it("Tillhub's instantiable and can create", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock
        .onPost(`https://api.tillhub.com/api/v0/business-partner-products/${legacyId}/${supplierId}/${productId}`)
        .reply(() => {
          return [
            200,
            {
    ...
    const { data, msg } = await SuppliersProductsRelation.create(supplierId, productId)
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      const th = await initThInstance()
      await th.suppliersProductsRelation().create(supplierId, productId)
    } catch (err: any) {
      expect(err.name).toBe('SuppliersProductsRelationCreationFailed')
    }
  })
})
