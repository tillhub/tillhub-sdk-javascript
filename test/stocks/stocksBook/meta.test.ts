...
describe('v0: StocksBook: can get meta', () => {
  it("Tillhub's stocksBook is instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onGet(`https://api.tillhub.com/api/v0/stock/${legacyId}/book/meta?${queryString}`).reply(() => {
        return [
          200,
          {
            results: [{ count: 900 }]
    ...
    const { data } = await stocksBook.meta(query)
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      const th = await initThInstance()
      await th.stocksBook().meta(query)
    } catch (err: any) {
