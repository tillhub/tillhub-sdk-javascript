...
describe('v0: Analytics: gets revenue sum for time range', () => {
  it("Tillhub's Analytics are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock
        .onGet(
          `https://api.tillhub.com/api/v0/analytics/${legacyId}/aggregates/revenues/sum?${queryString}`
        )
        .reply(() => {
          return [
            200,
    ...
    const { data } = await analytics.getRevenuesSumForTimeRange(queryObject)
