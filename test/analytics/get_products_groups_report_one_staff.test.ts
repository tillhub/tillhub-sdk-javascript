...
describe('v0: Analytics: gets product groups transactions report grouped by one staff member', () => {
  it("Tillhub's Analytics are instantiable", async () => {
    ...
    const { data } = await analytics.getProductGroupsReportOneStaff(staffMember)
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.analytics().getProductGroupsReportOneStaff(staffMember)
    } catch (err) {
      expect(err.name).toBe('ProductGroupsReportOneStaffFetchFailed')
    }
  })
})
