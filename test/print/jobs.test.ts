...
import { Jobs } from '../../src/v0/print/jobs'
import {
  PrintJobsFetchFailed,
  PrintJobFetchFailed,
...
const mockMsg = `Deleted job ${jobId}`
...
describe('v0: Print.Jobs', () => {
  it('retrieves all jobs', async () => {
    ...
    const jobs = th.print().jobs()

    expect(jobs).toBeInstanceOf(Jobs)
  ...
  it('creates one job', async () => {
    ...
    const { data } = await jobs.create(mockJob)
  ...
  it('updates one job', async () => {
    ...
    const { data } = await jobs.update(jobId, mockJob)
    expect(data).toEqual(mockJob)
  })
  ...
  it('deletes one job', async () => {
    ...
    const { msg } = await jobs.delete(jobId)
  ...
  it('retrieves job data', async () => {
    ...
    const { data } = await jobs.getData(jobId)
