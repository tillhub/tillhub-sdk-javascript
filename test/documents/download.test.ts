...
describe('v0: Documents: can get file', () => {
  it("Tillhub's documents are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock
        .onGet(`https://api.tillhub.com/api/v0/documents/${legacyId}/${documentId}/download`)
        .reply(() => {
          return [200, { results }]
        })
    }
    ...
    const { data, contentType, filename } = await documents.download(documentId)
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      const th = await initThInstance()
      await th.documents().download(documentId)
    } catch (err: any) {
