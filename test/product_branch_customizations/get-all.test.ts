...
describe('v0: Product branch customizations: can get all', () => {
  it("Tillhub's Product branch customizations are instantiable", async () => {
    ...
    const { data } = await ProductBranchCustomizations.getAll()
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.productBranchCustomizations().getAll()
    } catch (err: any) {
      expect(err.name).toBe('ProductBranchCustomizationsFetchAllFailed')
    }
  })
})
