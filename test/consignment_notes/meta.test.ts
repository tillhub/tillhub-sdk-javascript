...
describe('v0: Consignment Notes: can get count number of all consignment notes', () => {
  it("Tillhub's consignment Notes are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock
        .onGet(`https://api.tillhub.com/api/v0/consignment-notes/${legacyId}/meta?${queryString()}`)
        .reply(() => {
          return [
            200,
            {
    ...
    const { data } = await consignmentNotes.meta(query)
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.consignmentNotes().meta(query)
    } catch (err: any) {
      expect(err.name).toBe('ConsignmentNotesMetaFailed')
    }
  })
})
