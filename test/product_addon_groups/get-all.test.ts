...
describe('v0: Product addon groups: can get all', () => {
  it("Tillhub's Product addon groups are instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onGet(`https://api.tillhub.com/api/v0/product_addon_groups/${legacyId}`).reply(() => {
        return [
          200,
          {
            count: 1,
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      await th.productAddonGroups().getAll()
    } catch (err: any) {
      expect(err.name).toBe('ProductAddonGroupsFetchAllFailed')
    }
  })
})
