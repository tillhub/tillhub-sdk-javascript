...
describe('v1: StocksBook: can get meta', () => {
  it("Tillhub's stocksBook is instantiable", async () => {
    if (process.env.SYSTEM_TEST !== 'true') {
      ...
      mock.onGet(`https://api.tillhub.com/api/v1/stock/${legacyId}/book/meta?${queryString}`).reply(() => {
        return [
          200,
          {
            results: [{ count: 900 }]
    ...
    const stocksBookV1 = th.stocksBookV1()

    expect(stocksBookV1).toBeInstanceOf(v1.StocksBook)

    const { data } = await stocksBookV1.meta(query)
  ...
  it('rejects on status codes that are not 200', async () => {
    ...
    try {
      const th = await initThInstance()
      await th.stocksBookV1().meta(query)
    } catch (err: any) {
